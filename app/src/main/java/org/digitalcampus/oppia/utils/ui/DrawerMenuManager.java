...
public class DrawerMenuManager {
    ...
    public void onPrepareOptionsMenu(Integer currentOption, Map<Integer, MenuOption> options) {
        ...
        MenuItem itemLanguageDialog = drawerMenu.findItem(R.id.menu_langu);
    ...
    private void changeLanguageToTigrigna(AppActivity drawerAct) {

        Locale locale =new Locale("ti");
        Locale.setDefault(locale);
        Resources resources=drawerAct.getResources();
        Configuration configuration=resources.getConfiguration();
        configuration.locale=locale;
        resources.updateConfiguration(configuration,resources.getDisplayMetrics());
        Log.d("ChangeLanguage","Language is changing");
    ...
    public void showLanguageDialog(){
        Log.i("changeLanguage", "showLanguageDialog: ");
        AlertDialog.Builder builder = new AlertDialog.Builder(drawerAct);

        builder.setCancelable(false);
        builder.setTitle(R.string.lang);
        builder.setMessage("Change the Language Setting");
    ...
    private void changeLanguageToTigrna() {
        final String[] langues = {"English", "ትግርኛ","French","Arabic","Finnish","Spanish","Hindi","Urdu"};

        Log.d("ChangeLanguage", "Language is changingss");
        SharedPreferences preferences=drawerAct.getPreferences(MODE_PRIVATE);
        int lang_selected=preferences.getInt("lang_selected",1);
        final AlertDialog.Builder builder = new AlertDialog.Builder(drawerAct);
        builder.setTitle("Select A language").setSingleChoiceItems(langues, lang_selected, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {

                if (langues[which].equals("English")) {
                    changeLangu( "English", 0, "en");
                }
                if (langues[which].equals("ትግርኛ")) {
                    changeLangu( "Tig", 1, "ti");
                }

                if (langues[which].equals("French")) {
                    changeLangu( "French", 2, "fr");
                }
                if (langues[which].equals("Arabic")) {
                    changeLangu( "Arabic", 3, "ar");
                }
                if (langues[which].equals("Finnish")) {
                    changeLangu( "Finnish", 4, "fi");
                }
                if (langues[which].equals("Spanish")) {
                    changeLangu( "Spanish", 5, "es");
                }
                if (langues[which].equals("Hindi")) {
                    changeLangu( "Hindi", 6, "hi");
                }if (langues[which].equals("Urdu")) {
                    changeLangu( "Urdu", 7, "ur-rPK");
                }
    ...
    private void changeLangu(String tig, int i, String code) {

        SharedPreferences preferences = drawerAct.getPreferences(MODE_PRIVATE);
        SharedPreferences.Editor editor = preferences.edit();

        editor.putString("lang", code);
        editor.putInt("lang_selected", i);
        editor.apply();
